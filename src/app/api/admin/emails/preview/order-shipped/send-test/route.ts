...
import { buildOrderShippedFromLatestOrder } from "@/lib/email-templates/order-shipped-data";
...
export async function POST() {
  ...
  const result = await buildOrderShippedFromLatestOrder();
  if (!result.ok) {
    return NextResponse.json({ error: result.reason }, { status: 400 });
  }

  const { subject, html, text } = renderOrderShipped(result.data);
  ...
  const res = await fetch("https://api.postmarkapp.com/email", {
    ...
    body: JSON.stringify({
      ...
      Tag: "template-test-order-shipped",
    }),
  });
