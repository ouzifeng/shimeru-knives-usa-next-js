...
import { buildPartialRefundFromLatestOrder } from "@/lib/email-templates/order-status-data";
...
export async function POST() {
  ...
  const result = await buildPartialRefundFromLatestOrder();
  if (!result.ok) {
    return NextResponse.json({ error: result.reason }, { status: 400 });
  }

  const { subject, html, text } = renderOrderPartiallyRefunded(result.data);
  ...
  const res = await fetch("https://api.postmarkapp.com/email", {
    ...
    body: JSON.stringify({
      ...
      Tag: "template-test-order-partially-refunded",
    }),
  });

  if (!res.ok) {
    const details = await res.text();
    return NextResponse.json({ error: "Postmark send failed", details }, { status: 502 });
  }
