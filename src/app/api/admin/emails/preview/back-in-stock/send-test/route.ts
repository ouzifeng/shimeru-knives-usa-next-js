...
import { buildBackInStockPreview } from "@/lib/email-templates/back-in-stock-data";
...
export async function POST() {
  ...
  const result = await buildBackInStockPreview();
  if (!result.ok) {
    return NextResponse.json({ error: result.reason }, { status: 400 });
  }

  const { subject, html, text } = renderBackInStock(result.data);
  ...
  const res = await fetch("https://api.postmarkapp.com/email", {
    ...
    body: JSON.stringify({
      ...
      Tag: "template-test-back-in-stock",
      TrackOpens: true,
