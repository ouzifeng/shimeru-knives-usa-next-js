// Order refunded email, sent when a WC order flips to "refunded".
...
export function renderOrderRefunded(d: OrderStatusData): {
  subject: string;
  html: string;
  text: string;
} {
  const subject = `Your refund for order #${d.orderNumber}`;
...
<body style="margin:0;padding:0;background:${C.bg};font-family:${FONT_SANS};color:${C.foreground};-webkit-font-smoothing:antialiased;">
  <span style="display:none!important;visibility:hidden;opacity:0;color:transparent;height:0;width:0;font-size:1px;line-height:1px;max-height:0;max-width:0;overflow:hidden;">
    Your refund of ${escapeHtml(d.totalLabel)} for order #${escapeHtml(d.orderNumber)} is on its way.
  </span>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;">
          ...
          <tr>
            <td align="center" style="padding:0 24px 32px;">
              <h1 style="margin:0;font-family:${FONT_SERIF};font-weight:400;font-size:32px;line-height:1.2;color:${C.foreground};">Refund on its way</h1>
              <p style="margin:18px 0 0;font-family:${FONT_SANS};font-size:16px;line-height:1.6;color:${C.muted};">
                Hi ${escapeHtml(d.customerFirstName)}, we've refunded <strong style="color:${C.foreground};">${escapeHtml(d.totalLabel)}</strong> for order <strong style="color:${C.foreground};">#${escapeHtml(d.orderNumber)}</strong> back to your original payment method. Depending on your bank, it can take 5 to 10 business days to appear.
              </p>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.card};border:1px solid ${C.border};border-radius:4px;">
                <tr>
                  <td style="padding:22px 28px 6px;">
                    <div style="font-family:${FONT_SANS};font-size:10px;letter-spacing:0.22em;text-transform:uppercase;color:${C.muted};">Refunded order</div>
                ...
                <tr>
                  <td style="padding:14px 28px 24px;border-top:1px solid ${C.hairline};">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td style="font-family:${FONT_SANS};font-size:15px;font-weight:600;color:${C.foreground};">Refunded</td>
...
Refund on its way
Hi ${d.customerFirstName}, we've refunded ${d.totalLabel} for order #${d.orderNumber} back to your original payment method. Depending on your bank, it can take 5 to 10 business days to appear.

Refunded order #${d.orderNumber}
${itemsText}

Refunded: ${d.totalLabel}
