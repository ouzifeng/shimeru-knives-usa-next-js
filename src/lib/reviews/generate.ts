...
// Shared AI-review generation. Used by the manual admin endpoint
// (/api/admin/ai/generate-reviews) and the weekly cron
// (/api/cron/generate-reviews) so both produce identical output.
...
function detectProductType(product: { name?: string; short_description?: string; description?: string }): ProductType {
  const name = (product.name || "").toLowerCase();
  const desc = ((product.short_description || "") + " " + (product.description || "")).toLowerCase();
  const text = name + " " + desc;

  if (
    /\bset\b|\bkit\b|\bcollection\b|\b\d+\s*(piece|pc|knife|knives)\b/.test(name) ||
  ...
  if (
    /\bsharpener\b|\bsharpening\b|\bwhetstone\b|\bhoning\b|\bsteel\b.*\brod\b|\bstrop\b|\bblock\b|\bboard\b|\bcutting board\b|\bknife roll\b|\bknife bag\b|\bmagnetic\b.*\bstrip\b|\bknife stand\b|\bknife holder\b/.test(text)
  ) {
    return "accessory";
  }
...
function buildPrompt(
  product: { name?: string; price?: number; short_description?: string; description?: string; categories?: { name?: string }[] },
...
/** Load the configured AI provider/key/model from the settings table. */
export async function getAiSettings(admin: SupabaseClient): Promise<AiSettings | null> {
  ...
  if (!s.ai_provider || !s.ai_api_key || !s.ai_model) return null;
  return { ai_provider: s.ai_provider, ai_api_key: s.ai_api_key, ai_model: s.ai_model };
}

async function callProvider(provider: string, apiKey: string, model: string, prompt: string): Promise<string> {
...
/**
 * Generate `count` AI reviews for one product and push them to WooCommerce,
 * each stamped with a random date between dateFrom and dateTo.
 */
export async function generateAndPushReviews(
  admin: SupabaseClient,
  opts: { productId: number; count: number; dateFrom: string; dateTo: string; settings: AiSettings }
): Promise<GenerateOutcome> {
  const { productId, count, dateFrom, dateTo, settings } = opts;
  ...
  const attrSummary = attrs?.length
    ? Array.from(
        attrs.reduce((m: Map<string, string[]>, a: { attribute_name: string; attribute_value: string }) => {
  ...
  // Pre-compute ratings, grouped for the prompt.
  ...
  const raw = await callProvider(settings.ai_provider, settings.ai_api_key, settings.ai_model, prompt);
  ...
  const reviewItems: { rating: number; text: string }[] = Array.isArray(parsed)
    ? parsed.map((item: unknown, i: number) =>
        typeof item === "string"
          ...
          : {
              rating: (item as { rating?: number }).rating || ratings[i] || 5,
              text: (item as { text?: string; review?: string }).text || (item as { review?: string }).review || "",
            }
      )
    : [];

  if (!reviewItems.length) throw new Error("AI returned invalid review data");
